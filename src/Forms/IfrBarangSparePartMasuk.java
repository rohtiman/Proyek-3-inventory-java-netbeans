 ...
 * @author Atsutane
 */
public class IfrBarangSparePartMasuk extends javax.swing.JInternalFrame {

   Connection _Cnn;
    KoneksiDB getCnn = new KoneksiDB();
    
    String sqlinsert, sqlselect, sqldelete;
    String vno_barang_masuk, vid_user, vno_seri, vtgl_masuk, 
            vjml_masuk;
    
    DefaultTableModel tblsparepartmasuk;
     ...
     */
    public IfrBarangSparePartMasuk() {
        initComponents();
        clearInput(); disableInput(); listBarang(); listUser();
        setTabelSparePartMasuk(); showDataSparePartMasuk();
    }
    ...
    private void setTabelSparePartMasuk(){
        String[] kolom1 = {"No. Order", "Tanggal Masuk", "Barang", "Jumlah", "User"};
        tblsparepartmasuk = new DefaultTableModel(null, kolom1){
            ...
            public boolean isCellEditable(int row, int col){
                int cola = tblsparepartmasuk.getColumnCount();
                return (col < cola) ? false : true;
            }
        };
        tbDataBrgMasuk.setModel(tblsparepartmasuk);
        tbDataBrgMasuk.getColumnModel().getColumn(0).setPreferredWidth(75);
        tbDataBrgMasuk.getColumnModel().getColumn(1).setPreferredWidth(250);
        tbDataBrgMasuk.getColumnModel().getColumn(2).setPreferredWidth(25);
        tbDataBrgMasuk.getColumnModel().getColumn(3).setPreferredWidth(75);
        tbDataBrgMasuk.getColumnModel().getColumn(4).setPreferredWidth(75);
    }
    
    private void clearTableSparePartKeluar(){
        int row = tblsparepartmasuk.getRowCount(); //variabel row diberi nilai jumlah baris pada tabel(model) jurusan
        for(int i=0; i<row; i++){
            tblsparepartmasuk.removeRow(0); //menghapus record/baris
        }
    }
    
    private void showDataSparePartMasuk(){
        try {
            _Cnn = null;
            _Cnn = getCnn.getConnection();
            sqlselect = "select * from tbsparepart_masuk a, tbsparepart b, tb_user c where a.NOMOR_SERI=b.NOMOR_SERI and"
                    + " a.ID_USER=c.ID_USER ";
            Statement stat = _Cnn.createStatement();
            ResultSet res = stat.executeQuery(sqlselect);
            clearTableSparePartKeluar();
            while(res.next()){
                vno_seri = res.getString("NO_SPAREPART_MASUK");
                vtgl_masuk = tglview.format(res.getDate("TGL_SPAREPART_MASUK"));
                String vtipe_model = res.getString("NAMA_SPAREPART");
                vjml_masuk = res.getString("JML_SPAREPART_MASUK");
                String vid_user = res.getString("NAMA_USER");
                
                Object[] data = {vno_seri, vtgl_masuk, vtipe_model, vjml_masuk, vid_user};
                tblsparepartmasuk.addRow(data);
            }
    ...
    private void cariSparepartKeluar(){
        try{
             _Cnn = null;
             _Cnn = getCnn.getConnection();
             clearTableSparePartKeluar();
             sqlselect = "select * from tbsparepart like '%"+txtCari.getText()+"%'"
                     +"order by NAMA_MOTOR asc ";
             Statement stat = _Cnn.createStatement();
             ResultSet res = stat.executeQuery(sqlselect);
             while(res.next()){
                vno_barang_masuk = res.getString("NO_SPAREPART_MASUK");
                vtgl_masuk = tglview.format(res.getDate("TGL_SPAREPART_MASUK"));
                String vtipe_model = res.getString("NAMA_SPAREPART");
                vjml_masuk = res.getString("JML_SPAREPART_MASUK");
                String vid_user = res.getString("NAMA_USER");
    ...
    private void aksiSimpan(){
            vno_seri= txtNoBrgMasuk.getText();
            vno_seri = KeyBar[cmbBarang.getSelectedIndex()];
            ...
            try{
                ...
                if(btnSimpan.getText().equals("Simpan")){
                    sqlinsert = "insert into tbsparepart_masuk set NO_SPAREPART_MASUK='"+vno_seri+"', "
                            + " TGL_SPAREPART_MASUK='"+vtgl_masuk+"', NOMOR_SERI='"+vno_seri+"', JML_SPAREPART_MASUK='"+vjml_masuk+"', "
                            + " ID_USER='"+vid_user+"' ";
                    Statement stat = _Cnn.createStatement();
                ...
                btnHapus.setIcon(new javax.swing.ImageIcon(getClass().
                    ...
                    sqlinsert = "update tbsparepart_masuk set"
                            + " TGL_SPAREPART_MASUK='"+vtgl_masuk+"', NOMOR_SERI='"+vno_seri+"', JML_SPAREPART_MASUK='"+vjml_masuk+"', "
                            + " ID_USER='"+vid_user+"' "
                            + " where NO_SPAREPART_MASUK='"+vno_seri+"' ";
                    Statement stat =_Cnn.createStatement();
                stat.executeUpdate(sqlinsert);
                ...
                showDataSparePartMasuk(); clearInput(); disableInput();
                updateStokBarang();
    ...
    private void aksiHapus(){
        int jawab = JOptionPane.showConfirmDialog(this, "Apakah Anda akan menghapus data ini? kode"+vno_seri, 
                "Konfirmasi", JOptionPane.YES_NO_OPTION);
        if(jawab == JOptionPane.YES_OPTION){
            try{
                _Cnn = null;
                _Cnn = getCnn.getConnection();
                sqldelete = "delete from tbsparepart_masuk where NO_SPAREPART_MASUK = '"+vno_seri+"'";
                Statement stat = _Cnn.createStatement();
    ...
    private void updateStokBarang(){
        int jml=0, stok=0;
        sqlselect = " select STOK_SPAREPAR from tbsparepart where NOMOR_SERI='"+vno_seri+"' ";
        ...
        try {
            ...
            ResultSet res = stat.executeQuery(sqlselect);
            while(res.next()){
                jml = Integer.parseInt(res.getString(1));
            }
            stok = jml + Integer.parseInt(vjml_masuk);
            sqlinsert = " update tbsparepart set STOK_SPAREPAR='"+stok+"' where NOMOR_SERI='"+vno_seri+"' ";
            try {
                _Cnn = getCnn.getConnection();
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setTitle("Form Barang Sparepart Masuk");
        ...
        jLabel8.setText("Form Barang Sparepart Masuk");

        jLabel9.setText("Form ini digunakan untuk mengolah data sparepart masuk");
