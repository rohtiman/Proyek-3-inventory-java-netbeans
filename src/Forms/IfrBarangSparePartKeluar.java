 ...
 * @author Atsutane
 */
public class IfrBarangSparePartKeluar extends javax.swing.JInternalFrame {
    ...
    String vno_order, vid_user, vno_seri, vtgl_keluar, 
            vjml_keluar;
    
    DefaultTableModel tblsparepartkeluar;
     ...
     */
    public IfrBarangSparePartKeluar() {
        initComponents();
        
        clearInput(); disableInput(); listBarang(); listUser();
        setTabelSparePartKeluar(); showDataSparePartKeluar();
    }
    ...
    private void setTabelSparePartKeluar(){
        String[] kolom1 = {"No. Order", "Tanggal Keluar", "Barang", "Jumlah", "User"};
        tblsparepartkeluar = new DefaultTableModel(null, kolom1){
            ...
            public boolean isCellEditable(int row, int col){
                int cola = tblsparepartkeluar.getColumnCount();
                return (col < cola) ? false : true;
            }
        };
        tbDataBrgKeluar.setModel(tblsparepartkeluar);
    ...
    private void clearTableSparePartKeluar(){
        int row = tblsparepartkeluar.getRowCount(); //variabel row diberi nilai jumlah baris pada tabel(model) jurusan
        for(int i=0; i<row; i++){
            tblsparepartkeluar.removeRow(0); //menghapus record/baris
        }
    }
    
    private void showDataSparePartKeluar(){
        try {
            _Cnn = null;
            _Cnn = getCnn.getConnection();
            sqlselect = "select * from tbsparepart_keluar a, tbsparepart b, tb_user c where a.NOMOR_SERI=b.NOMOR_SERI and"
                    + " a.ID_USER=c.ID_USER ";
            Statement stat = _Cnn.createStatement();
            ResultSet res = stat.executeQuery(sqlselect);
            clearTableSparePartKeluar();
            while(res.next()){
                vno_order = res.getString("NO_SPAREPART_KELUAR");
                vtgl_keluar = tglview.format(res.getDate("TGL_SPAREPART_KELUAR"));
                String vtipe_model = res.getString("NAMA_SPAREPART");
                vjml_keluar = res.getString("JML_SPAREPART_KELUAR");
                String vid_user = res.getString("NAMA_USER");
                
                Object[] data = {vno_order, vtgl_keluar, vtipe_model, vjml_keluar, vid_user};
                tblsparepartkeluar.addRow(data);
            }
        ...
        if(txtNoOrder.getText().equals("")){
            ...
            try{
                ...
                if(btnSimpan.getText().equals("Simpan")){
                    sqls = "select STOK_SPAREPAR from tbmotor where NOMOR_SERI='"+vno_seri+"' ";
                    ...
                    sqlinsert = "insert into tbsparepart_keluar set NO_SPAREPART_KELUAR='"+vno_order+"', "
                            + " TGL_SPAREPART_KELUAR='"+vtgl_keluar+"', NOMOR_SERI='"+vno_seri+"', JML_SPAREPART_KELUAR='"+vjml_keluar+"', "
                            + " ID_USER='"+vid_user+"' ";
                    Statement stat = _Cnn.createStatement();
                ...
                btnHapus.setIcon(new javax.swing.ImageIcon(getClass().
                    ...
                    sqlinsert = "update tbsparepart_keluar set"
                            + " TGL_SPAREPART_KELUAR='"+vtgl_keluar+"', NOMOR_SERI='"+vno_seri+"', JML_SPAREPART_KELUAR='"+vjml_keluar+"', "
                            + " ID_USER='"+vid_user+"' "
                            + " where NO_SPAREPART_KELUAR='"+vno_order+"' ";
                    Statement stat =_Cnn.createStatement();
                stat.executeUpdate(sqlinsert);
                ...
                showDataSparePartKeluar(); clearInput(); disableInput();
                updateStokBarang();
    ...
    private void aksiHapus(){
        ...
        if(jawab == JOptionPane.YES_OPTION){
            try{
                _Cnn = null;
                _Cnn = getCnn.getConnection();
                sqldelete = "delete from tbsparepart_keluar where NO_SPAREPART_KELUAR = '"+vno_order+"'";
                Statement stat = _Cnn.createStatement();
    ...
    private void updateStokBarang(){
        ...
        try {
            ...
            stok = jml - Integer.parseInt(vjml_keluar);
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setTitle("Form Barang Sparepart Keluar");
        ...
        jLabel8.setText("Form Barang Sparepart Keluar");

        jLabel9.setText("Form ini digunakan untuk mengolah data sparepart keluar");
